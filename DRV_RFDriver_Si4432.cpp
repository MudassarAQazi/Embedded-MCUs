...
HAL_StatusTypeDef RFDriver_Si4432::RFDriver_begin(	SPI_HandleTypeDef*	hspi,
...
//	this->setOperatingMode(this->SENSOR);
	...
	return this->reloadConfigurations();
}
...
//#endif
	...
	/// Set All Interrupt to Read and Clear All Interrupt for Smooth Operation.
	this->setInterrupts(this->INT_ALL);
	#if !defined(noDelay)
		HAL_Delay(50);
	#endif
...
//	#endif
	...
	//Currently Set to Pre-Amble: 93 Nibbles = 372 bits, Threshold: 8 Nibbles = 32 bits.
    this->setPreamble(10, 7); // Required At Least (Pre-Amble: 10 Nibbles = 40 bit, Threshold: 7 Nibbles = 28 bits).
	#if !defined(noDelay)
		HAL_Delay(10);
	#endif
...
//    uint8_t synchWord[] = {0x2D, 0xD4};
    ...
    this->setTxPower(11); // Max is 20.
		...
		this->setFrequency(433.00, 0, 0, 0x64); /** (frequency, frequencyOffset, channel, stepSize) **/ //stepSize: 0x64
	...
	#ifdef freq915M
		/// Set Frequency + Set Frequency Offset + Set Channel + Step Size (1 MHz)
		this->setFrequency(915.00, 0, 0, 0x64); /** (frequency, frequencyOffset, channel, stepSize) **/ //stepSize: 0x64
		//  RF.setFrequencyHoppingChannel(10, 1);
		#if !defined(noDelay)
			HAL_Delay(10);
		#endif
...
HAL_StatusTypeDef RFDriver_Si4432::readAndClearInterrupts(uint16_t* interrupts)
{
	...
	*/
	/// Local Variables.
	uint8_t interruptStatusReg1Addr	=	0x03;	/* 0x03 Interrupt Status 1 Register Address */
//	uint8_t intRegSize	=	0x02;

	/// Read two bytes of interrupts from Register 03h & 04h from SPI.
	this->status = this->read_Reg_From_RF_SPI(&interruptStatusReg1Addr, txRxReg, 2);
...
///-29-
/// Function to Get RSSI Value of the Received Signals.
HAL_StatusTypeDef RFDriver_Si4432::getRSSI(int8_t* rssiValue)
{
	/*
		Function to Get RSSI Value.
	...
	*/
	...
	int8_t	rssiValueReg		=	0;

	/// Read RSSI Register (26h) to SPI.
	this->status = this->read_Reg_From_RF_SPI(&rssiValueRegAddr, (uint8_t*)&rssiValueReg, sizeof(rssiValueReg));

	/// Convert Binary to dB.
	*rssiValue	=	(int8_t)((float)(rssiValueReg * 0.5));
...
///-32-
///
//uint8_t RFDriver_Si4432::sendDataPacket(uint8_t* data, uint8_t size)
//{
//	/*
...
//		if(HAL_GPIO_ReadPin(this->nIrqPort, this->nIrqPin) == GPIO_PIN_SET) continue;
//
//		/// Read Interrupts.
//		this->readAndClearInterrupts();
//
...
/////-32-
/////
//uint8_t RFDriver_Si4432::sendDataPacket(uint8_t* data, uint8_t size, bool waitForAckResponse, uint32_t responseTimeout, uint8_t* responseBuffer, uint8_t* responseSize)
//{
//	/*
//		Function to Send Data from the RF Module.
//		-	*data:		Data to Write on the Transmit Registers.
...
//uint8_t RFDriver_Si4432::waitForAckPacket(uint32_t responseTimeout)
//{
//	/*
//		Function to Send Data from the RF Module.
...
//
//	while (((timeStampWait - enterTimeWait) <= responseTimeout))// && (!packReceived))
////	do
//	{
//		HAL_Delay(0);
//
...
//
//		/// Check for Packet.
////		packReceived = this->isDataPacketReceived();
//		if (this->isDataPacketReceived())	return 1U;
//		else	continue;
//	}
////	} while (((timeStampWait - enterTimeWait) <= responseTimeout) && (!packReceived));
//
////	if (packReceived)	return 1;
//
...
//uint8_t RFDriver_Si4432::isDataPacketReceived()
//{
//	/*
...
//	if(HAL_GPIO_ReadPin(this->nIrqPort, this->nIrqPin) == GPIO_PIN_SET) return 0;
//
//	/** Read and Clear Interrupts **/
//	this->readAndClearInterrupts();
//
...
HAL_StatusTypeDef RFDriver_Si4432::readDataFromRF(uint8_t* fifo, uint8_t* size)
{
	...
	*/
	...
	/// Make the copy of Packet Length Variable.
	//rxPacketLength	=	rxPacketLengthReg;
...
HAL_StatusTypeDef RFDriver_Si4432::write_Reg_To_RF_SPI(uint8_t* address, uint8_t* data, size_t size)
{
	...
	*/
	...
	/// Write Register.
	this->status = HAL_SPI_Transmit(rf_spi, &writeAddress, 1, 1);
	/// Malfunction Action.
	if(this->status != HAL_OK) { HAL_GPIO_WritePin(this->nSelPort, this->nSelPin, GPIO_PIN_SET);	return this->status; }

	/// Write Register.
	this->status = HAL_SPI_Transmit(rf_spi, data, size, 3);
	/// Malfunction Action.
...
HAL_StatusTypeDef RFDriver_Si4432::read_Reg_From_RF_SPI(uint8_t* address, uint8_t* data, size_t size)
{
	...
	*/
	...
	/// Read Register.
	this->status = HAL_SPI_Transmit(rf_spi, &readAddress, 1, 1);
	/// Malfunction Action.
	if(this->status != HAL_OK) { HAL_GPIO_WritePin(this->nSelPort, this->nSelPin, GPIO_PIN_SET);	return this->status; }

	/// Read Register.
	this->status = HAL_SPI_Receive(this->rf_spi, data, size, 50); // 3
	/// Malfunction Action.
	if(this->status != HAL_OK) { HAL_GPIO_WritePin(this->nSelPort, this->nSelPin, GPIO_PIN_SET);	return this->status; }
